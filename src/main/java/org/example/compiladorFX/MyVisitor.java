...
public class MyVisitor extends gBaseVisitor<Object> {
    ...
    public MyVisitor(TextArea textout) {
    ...
    Object evalCondition(Object left, String operator, Object right) {
        if (left instanceof Number && right instanceof Number) {
            ...
            System.err.println("Error de tipos en comparacion : " + left + " " + operator + " " + right);
        ...
        if (isDefined(id)) {
            ...
            if (varType.equals("int") && !(value instanceof Integer)) {
                if (((Number)value).doubleValue() != Math.rint(((Number)value).doubleValue())){
                    System.err.println("Error de tipo: No se puede asignar un valor no entero a variable int '" + id + "'");
                    return false;
                }else{return true;}
            } else if (varType.equals("double") && !(value instanceof Double || value instanceof Integer)) { // Allow int to double conversion
                System.err.println("Error de tipo: No se puede asignar un valor no numerico a variable double '" + id + "'");
                return false;
            }
        }
        return true;
        ...
        if (isDefined(id)) {
            ...
            if (ctx.math_expression() != null) { // Declaration with initialization
                ...
                if (checkAssignmentCompatibility(id, value)) {
                    ...
                    System.out.println("Declaracion de variable: " + symbolTableStack.peek().get(id));
                }
    ...
    public Object visitVariable_assign(gParser.Variable_assignContext ctx) {
        ...
        if (!isDefined(id)) {
            ...
            if (checkAssignmentCompatibility(id, value)) {
                ...
                System.out.println("Asignacion de variable: " + symbolTableStack.peek().get(id));
            }
        }
        return null;
    }
        ...
        if (ctx.ID() != null) { // Factor is an ID (variable)
            ...
            if (!isDefined(id)) {
                ...
                if (val == null) {
                    ...
                    System.err.println("Error: Variable '" + id + "' no es una variable numerica");
                    return 0.0;
                }
            }
    ...
    public Object visitBoolean(gParser.BooleanContext ctx) {
        if (ctx.math_expression() != null) { // Comparison between math expressions
